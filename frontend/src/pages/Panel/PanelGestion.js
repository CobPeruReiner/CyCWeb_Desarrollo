...
export const PanelGestion = (props) => {
    ...
    const rowClass = (rowData) => {
        return {
            "state-row status-4naranja": rowData.COLOR === "4naranja",
            "state-row status-autorizado": rowData.IDAUTORIZATEL === 1,
            "state-row status-no-autorizado": rowData.IDAUTORIZATEL === 2,
        };
    };
    ...
    const handleSelected = (e, entityId, carteraId) => {
        ...
        //         severity: "warn",
        //         summary: "Advertencia",
        //         detail: "Debe seleccionar una cartera antes de consultar los teléfonos",
        //         life: 4000,
        //     });
    ...
    useEffect(() => {
        ...
        const oLogin = localStorageService.getUserLogin();

        if (!oLogin || !oLogin.clients || oLogin.clients.length === 0) {
            console.warn("El usuario no tiene carteras configuradas.");
            setCustomers([]);
            setNameCliente("Sin carteras configuradas");
            panelContext.setSelectedEntityId(null);
            panelContext.setSelectedCarteraId(null);
            return;
        }
        ...
        if (entityId != null) {
            ...
            if (!currentIdCarteraItem) {
                console.warn("No se encontró cartera asociada al entityId actual.");
                setNameCliente("Sin cartera válida");
                panelContext.setSelectedEntityId(null);
                panelContext.setSelectedCarteraId(null);
                return;
            }

            localStorageService.updateInfo({
                ...oLogin,
                entityId: entityId,
                carteraId: currentIdCarteraItem.idcartera,
            });

            panelContext.setSelectedEntityId(entityId);
            panelContext.setSelectedCarteraId(currentIdCarteraItem.idcartera);
            setNameCliente(currentIdCarteraItem.nombre ?? "Cartera sin nombre");
            ...
            if (oLogin.hasOwnProperty("entityId")) {
                ...
                tempEntityId = oLogin.clients?.[0]?.id_tabla ?? null;
                tempCarteraId = oLogin.clients?.[0]?.idcartera ?? null;
            }

            if (!tempEntityId || !tempCarteraId) {
                console.warn("No hay carteras disponibles para inicializar.");
                setNameCliente("Sin carteras configuradas");
                panelContext.setSelectedEntityId(null);
                panelContext.setSelectedCarteraId(null);
                return;
            ...
            localStorageService.updateInfo({
                ...
                carteraId: tempCarteraId,
            });

            const cliente = oLogin.clients.find((r) => r.id_tabla == tempEntityId);
            setNameCliente(cliente?.nombre ?? "Cartera sin nombre");
                    ...
                    gestionservice.getFilter(paramIdentity, tempEntityId).then((data) => {
                        ...
                        let arr = data.result.reduce((hash, obj) => {
                            ...
                            return Object.assign(hash, {
                                [obj["NOMBRE"]]: (hash[obj["NOMBRE"]] || []).concat(obj.identificador),
                            });
                        }, {});
                        ...
                        if (data.result.length > 0) {
                            ...
                            console.warn("Búsqueda temporal en", tempEntityId);
                            toast.current.show({
                                severity: "warn",
                                summary: "Información",
                                detail: "No se encontró información para el identificador " + paramIdentity,
                            });
                        }
                    });
                }
    ...
    const esCastigo = panelContext.userLogin.clients.some((c) => Number(c.idcartera) === Number(panelContext.selectedCarteraId) && Number(c.tipo_cartera) === 3);
                                                            ...
                                                            <div key={"key_panel_3_" + user.field} className="p-col-4 p-p-0">
                                                                <div className="p-grid p-col p-p-1">
                                                                    ...
                                                                    <label
                                                                        ...
                                                                        style={{
                                                                            background:
                                                                                panelContext.selectedCarteraId === 51 && user.field === "EMAIL" && selectedProduct[user.field] === "FRANK"
                                                                                    ? "#e3f2fd"
                                                                                    : panelContext.selectedCarteraId === 51 && user.field === "EMAIL" && selectedProduct[user.field] === "EDUARDO"
                                                                                      ? "#e8f5e9"
                                                                                      : (user.color && user.header !== "ESTADO") || (user.header === "ESTADO" && selectedProduct[user.field]?.toUpperCase() === "VENCIDO")
                                                                                        ? user.color
                                                                                        : panelContext.selectedCarteraId === 70 && user.field === "NOMBRE"
                                                                                          ? "yellow"
                                                                                          : "",
                                                                            color:
                                                                                panelContext.selectedCarteraId === 51 && user.field === "EMAIL" && selectedProduct[user.field] === "FRANK"
                                                                                    ...
                                                                                    : panelContext.selectedCarteraId === 51 && user.field === "EMAIL" && selectedProduct[user.field] === "EDUARDO"
                                                                                      ? "#2e7d32"
                                                                                      : "inherit",
                                                                            fontWeight: panelContext.selectedCarteraId === 51 && user.field === "EMAIL" ? "bold" : "normal",
                                                                            fontSize: "11.5px",
                                                                            textAlign: "start",
                                                                        }}
                                                                    >
                                    ...
                                    >
                                        ...
                                        <Dropdown
                                            ...
                                            options={[
                                                ...
                                                { label: "ENRIQUECIMIENTO", value: "ENRIQUECIMIENTO" },
